...
CREATE TABLE position (
  position_id INT(11) NOT NULL AUTO_INCREMENT, -- Unique identifier for each position (auto-incremented)
  description VARCHAR(45) NOT NULL,           -- Description of the position
...
CREATE TABLE department (
  department_id INT(11) NOT NULL AUTO_INCREMENT, -- Unique identifier for each department (auto-incremented)
  name VARCHAR(45) NOT NULL,                    -- Name of the department
...
CREATE TABLE employee (
  employee_id INT(11) NOT NULL AUTO_INCREMENT, -- Unique identifier for each employee (auto-incremented)
  name VARCHAR(60) NOT NULL,                  -- Name of the employee
  salary DECIMAL(8,2) NOT NULL,               -- Salary of the employee
  position_id INT(11) NOT NULL,               -- Foreign key reference to 'position' table
  department_id INT(11) NOT NULL,             -- Foreign key reference to 'department' table
  PRIMARY KEY (employee_id),                  -- Primary key constraint on 'employee_id'
  INDEX fk_employee_position_idx (position_id ASC), -- Index on 'position_id' for faster lookups
  INDEX fk_employee_department1_idx (department_id ASC), -- Index on 'department_id' for faster lookups
  CONSTRAINT fk_employee_position FOREIGN KEY (position_id) 
    REFERENCES position (position_id),        -- Foreign key constraint linking 'position_id' to 'position' table
  CONSTRAINT fk_employee_department1 FOREIGN KEY (department_id)
    REFERENCES department (department_id)     -- Foreign key constraint linking 'department_id' to 'department' table
);
...
VALUES 
('John Smith', 8450.50, 1, 4),         -- Employee details with references to 'position' and 'department'
